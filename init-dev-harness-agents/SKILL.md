...
**知识透明原则**：用户作为决策者需要理解框架的设计理念、方法论与运作机制，才能有效操控并判断决策对错——本 skill 不做黑盒。决策点、方法论基础、角色协作逻辑、决策可错性判断依据全部对用户可见。下面的人（角色）做好自测，上面的人（architect/orchestrator）做好规划和监督。大决策用户明确（少而精），小决策角色自决（多而快）。

**为什么这样设计**（用户必读，理解后才能有效操控）：
- **五条方法论融合**：总经理谨慎经营（瀑布）+ 大将军指挥千军万马（Scrum）+ Martin Fowler 渐进式 + 小步快跑 + TDD——分别对应"决策谨慎、执行高效、演进可控、迭代快速、质量保证"五个维度，缺一不可
- **6 角色而非更多**：architect（决策）/ orchestrator（调度）/ executor（实施）/ reviewer（审查）/ devops（交付）/ retro-optimizer（飞轮）——覆盖软件工程全生命周期，retro-optimizer 是知识库自形成的关键
- **决策分层而非集中**：大决策少而精（用户定方向），小决策多而快（角色定细节）——避免用户被细节淹没，也避免角色越权定方向
- **决策可错性**：老板和大模型都会犯错，每个决策标注可证伪条件，retro-optimizer 在 Sprint 回顾时检验——不盲从任何层级
...
初始化时一次性收集，默认值优先，不阻塞流程。每个决策项附**决策依据**，用户理解后可判断默认值是否合适：

| 决策项 | 默认值（用户不答时） | 决策依据（为什么这样默认） |
|---|---|---|
| 项目目标一句话 | 从 package.json/README 推断 | 推断值作为起点，首次 Sprint 回顾时由 architect 校准 |
| 技术栈确认 | 扫描依赖文件自动识别 | 实际依赖比口头描述更准确；architect 在 wf_scope 校准 |
| 是否启用 dual-phase-dev | 不启用（用 standard-dev） | 双阶段（瀑布+Scrum）是重型工作流，适合需求明确的大型项目；standard-dev 先验证基础协作，需求稳定后再升级 |
| Agent 角色范围 | 启用 6 角色（含 retro-optimizer 飞轮） | retro-optimizer 是知识库自形成的核心，缺则无飞轮；6 角色覆盖软件工程全生命周期 |
| 大周期节奏 N | N=10（每 10 轮交互触发深度回顾） | 平衡频率与深度：太频繁（<5）变成小步快跑，太稀疏（>20）遗漏细节；10 轮足以积累可提炼的模式 |
| 通信超时/重试 | 30s / 3 次 | 平衡响应与容错：30s 覆盖大多数 LLM 调用；3 次重试避免单点失败，超过则升级 orchestrator |
